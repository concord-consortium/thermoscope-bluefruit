...
int32_t tsServiceIdB;
int32_t tsMeasureCharIdB;
...
void setup(void) {
  ...
  tsServiceIdB = gatt.addService(sensorBServiceUUID);
  if (tsServiceIdB == 0) {
    error(F("Could not add service"));
  }
  ...
  // We are going to try sending the temperatue as a 2byte 16bit integer with a exponent of -2
  // so a temperatue of 30.01 which will be sent as 3001
  tsMeasureCharIdB = gatt.addCharacteristic(
    0x0001, // UUID
    GATT_CHARS_PROPERTIES_READ | GATT_CHARS_PROPERTIES_NOTIFY, // read and notify
    4, // min len
    4, // max len
...
void readTemperature(int32_t gattCharID, const __FlashStringHelper*label, int32_t pin){
  uint8_t i;
  float average;
  ...
  gatt.setChar(gattCharID, (int32_t)(steinhart * 100));
...
void loop(void) {
  ...
  readTemperature(tsMeasureCharIdA, F("A"), THERMISTORPINA);
  readTemperature(tsMeasureCharIdB, F("B"), THERMISTORPINB);
