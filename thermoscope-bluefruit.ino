...
int32_t addCoefficientCharacteristic(uint16_t uuid, uint16_t value,
  ...
  ble.setBleGattRxCallback(id, gattCallback);
...
//  ble.atcommand(F("AT+DBGNVMRD"));
  ...
  // add a callback so we get notified when this changes
  ble.setBleGattRxCallback(idStringCharId, BleGattRX);
